-- Adapted for MultiBeam from BeamMP (https://github.com/BeamMP/BeamMP), nodesVE.lua.
...
local function getNodes()
  ...
  for _, beam in pairs(v.data.beams) do
	...
	obj:queueGameEngineLua("multibeam_client.sendNodes(\'"..jsonEncode(save).."\', "..obj:getID()..")") -- Send it to GE lua
end
...
local function applyNodes(data)
  ...
  --[[if not decodedData or decodedData.nodeCount ~= #v.data.nodes then --or decodedData.beamCount ~= #v.data.beams then
    log("E", "mbNodesVE", "unable to use nodes data.")
    return
  end]]
...
local function getBreakGroups()
	...
	obj:queueGameEngineLua("multibeam_client.sendBreakGroups(\'"..jsonEncode(breakGroupArray).."\', "..obj:getID()..")") -- Send it to GE lua
end
