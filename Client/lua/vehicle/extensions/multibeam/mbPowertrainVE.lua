-- Adapted for MultiBeam from BeamMP (https://github.com/BeamMP/BeamMP), MPPowertrainVE.lua.
...
local function getPowerTrainData()
	...
	for k, v in pairs(devices) do -- For each device
		if v.mode and v.setMode then -- If it's something that interests us
			...
			-- If it has changed
			-- We don't send the gearbox because it's already synced into mbElectricsVE
	...
	if next(devicesToSend) then
		obj:queueGameEngineLua("multibeam_client.sendLivePowertrain(\'"..jsonEncode(devicesToSend).."\', "..obj:getID()..")")
...
local function getEngineData() --TODO maybe hook the functions instead of checking with ticking
	...
	if next(data) then
		obj:queueGameEngineLua("multibeam_client.sendEngineData(\'"..jsonEncode(data).."\', "..obj:getID()..")")
	end
end
