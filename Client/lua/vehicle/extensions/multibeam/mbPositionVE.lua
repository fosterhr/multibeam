-- Adapted for MultiBeam from BeamMP (https://github.com/BeamMP/BeamMP), positionVE.lua.
...
local function update(dtSim)
	...
	if physcounter == 2000 then
		...
		if playerInfo.firstPlayerSeated then
			...
			obj:queueGameEngineLua("multibeam_client.setActualSimSpeed("..tostring(physmult)..")")
		end
	end
...
local function updateGFX(dt)
	...
	local cog = mbVelocityVE.cogRel:rotated(vehRot)
	...
	if framesSinceReset > 5 then -- wating 6 frames then always teleporting the 6th frame makes reseting/recovering a remote vehicle at speed teleport much more consistent, maybe the smoothers catching up?
		if framesSinceReset == 6 or tpTimer > (tpDelayAdd + abs(predictTime)) or posErrorLen > tpDist2 or rotErrorLen > tpRot2 then
			...
			local tpPos = pos - mbVelocityVE.cogRel:rotated(rot)
			...
			obj:queueGameEngineLua("multibeam_client.setPositionRotationVelocity("..obj:getID()..","..serialize(posData)..")")
	...
	if framesSinceReset > 5 then
		if targetRacc:length() > minRotForce or vehVel:length() > 1 then
			mbVelocityVE.addAngularVelocity(targetAcc.x, targetAcc.y, targetAcc.z, targetRacc.x, targetRacc.y, targetRacc.z)
		elseif targetAcc:length() > minPosForce then
			mbVelocityVE.addVelocity(targetAcc.x, targetAcc.y, targetAcc.z)
		end
	end
...
local function getVehicleRotation()
	...
	local cog = mbVelocityVE.cogRel:rotated(rot)
	...
	obj:queueGameEngineLua("multibeam_client.sendVehiclePosRot(\'"..jsonEncode(tempTable).."\', "..obj:getID()..")") -- Send it
end
